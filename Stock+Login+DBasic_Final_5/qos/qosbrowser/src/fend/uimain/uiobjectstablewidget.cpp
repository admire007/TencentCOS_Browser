...
UiObjectsTableWidget::UiObjectsTableWidget(QWidget *parent) : QWidget(parent),
...
void UiObjectsTableWidget::on_btnUpload_clicked()
{
    ...
    if (info.isFile() && info.exists())
    {
        ...
        // 记录文件选择完成时间
        qint64 selectTime = timer.elapsed();
        qDebug() << "文件选择耗时:" << selectTime << "ms";
...
void UiObjectsTableWidget::onUploadSuccess(const QString &jobId)
{
    ...
    showMessage(
        QString::fromLocal8Bit("上传文件"),
        QString::fromLocal8Bit("上传文件成功"));
}

void UiObjectsTableWidget::onDownloadSuccess(const QString &jobId)
{
    showMessage(
        QString::fromLocal8Bit("下载文件"),
