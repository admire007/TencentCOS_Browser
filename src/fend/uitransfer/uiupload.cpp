...
UiUpload::UiUpload(QWidget *parent) : QWidget(parent),
                                      ui(new Ui::UiUpload)
{
    ui->setupUi(this);
    initHeader();
...
void UiUpload::onStartUpload(const QString &jobId, const QString &key, const QString &localPath)
{
    ...
    // 记录进度条显示完成时间
    qint64 progressTime = timer.elapsed();
    qDebug() << "进度条显示耗时:" << progressTime << "ms";
}

void UiUpload::onUploadProcess(const QString &jobId, qulonglong transferedSize, qulonglong totalSize)
{
    static QMap<QString, QElapsedTimer> timers;
    ...
    // 记录首次进度更新时间
    if (transferedSize > 0 && transferedSize <= 1024 * 512)
    {
        qint64 firstUpdateTime = timers[jobId].elapsed();
        qDebug() << "首次进度更新耗时:" << firstUpdateTime << "ms";
    }
}
